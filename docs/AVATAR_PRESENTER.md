...
- **WebGL context budget**: each mounted `TalkingHeadAvatar` is its own WebGL context. `MAX_CONCURRENT_HEADS` in `talkingHeadMotion.js` caps live instances at 3; beyond that the 2D SVG is shown. A `webglcontextlost` listener uses the same fallback. The chat header avatar frees its slot while a call is open.
...
- **First frame**: `loadTalkingHead` waits two `requestAnimationFrame` ticks after `showAvatar()` resolves before the canvas is revealed.
