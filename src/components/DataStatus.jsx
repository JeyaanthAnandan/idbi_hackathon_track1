...
export default function DataStatus() {
  ...
  const demo = sources.some((s) => /^(sandbox|synthetic):/.test(s)) || !boot.session;
  return (
    <div className="web-span" role="status" style={{ padding: '12px 18px', fontSize: 12, lineHeight: 1.6, color: 'var(--ink-soft)', background: 'var(--card)', borderRadius: 12 }}>
      <strong>{demo ? 'Demo / sandbox data' : boot.profile ? 'Uploaded financial data' : 'No financial data connected'}</strong>
