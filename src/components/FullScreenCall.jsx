...
export default function FullScreenCall({ messages, caption, heard, interim, speaking, listening, transcribing, typing, preparing, secs, error, voiceOn, micLevel, direction, onSend, onMic, onMute, onEnd, onPause, onExplain, completion = 0 }) {
  ...
  const line = useGuideTarget(focus, avatarRef);
  ...
  const gesture = celebrating ? 'thumbs-up' : listening ? 'listen' : focus ? left ? 'point-left' : 'point-right' : manualGesture || 'idle';
  ...
  return <section ref={rootRef} className="mitra-call-room" role="dialog" aria-modal="true" aria-label="Full-screen MITRA conversation" data-surface="night" onKeyDown={(e) => {
    ...
    <div className="mitra-call-workspace">
      <aside className="mitra-call-guide"><div ref={avatarRef}><Avatar size={270} stage speaking={speaking} mood={celebrating ? 'excited' : typing ? 'thinking' : 'happy'} gesture={gesture} gaze={focus ? left ? 'left' : 'right' : 'center'} elevation={elevation} /></div><h2>{person.name}</h2><p role="status">{status}</p><div className="mitra-call-meter" aria-label={listening ? 'Microphone level' : 'Voice status'}>{[.4,.75,1,.6,.9,.5,.8].map((v,i) => <i key={i} style={{ height: `${6 + (listening ? micLevel : speaking ? .5 : .08) * v * 35}px` }} />)}</div><p className="mitra-call-hint">I’ll bring up the chart and show you where to look.</p><button type="button" className="ghost-btn" onClick={onPause}>Pause voice & microphone</button></aside>
      <div className="mitra-call-content">
        ...
        <div className={`mitra-call-panels ${widget && chartVisible ? 'has-chart' : ''}`}>
          ...
          {widget && chartVisible && <section className="mitra-call-chart" data-guide-target="call-chart" aria-label="Chart for the current answer"><div className="mitra-call-panel-heading"><h2>Let me show you</h2><button type="button" onClick={() => setFocus('call-chart')}>Point here ↙</button></div><ChatWidget widget={widget} onChip={submit} />{scene && <div className="mitra-call-bars"><h3>{scene.title}</h3><p>Select a bar and I’ll explain it.</p><PresenterBars scene={scene} target={focus?.replace('call-bar-', '')} guidePrefix="call-bar-" disabled={typing || transcribing} onSelect={(index) => {
    ...
    {line && <svg className="mitra-guide-line" aria-hidden="true"><path d={`M${line.x1} ${line.y1} Q${line.x1 + (line.x2-line.x1)*.6} ${line.y1} ${line.x2} ${line.y2}`} /><circle cx={line.x2} cy={line.y2} r="5" /></svg>}
  </section>;
}
